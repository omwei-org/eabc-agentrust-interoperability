...
HOOK = '''        # T30.3 EXPERIMENTAL: EABC admission hook. This is repository-added
        ...
        cls = type(self)
        if not hasattr(cls, "_t30_3_reservation_lock"):
            import threading
            cls._t30_3_reservation_lock = threading.Lock()
            cls._t30_3_reserved_execution_ids = set()
        if execution_id is not None and t30_3_adapter is not None and t30_3_hook_enabled:
            ...
            with cls._t30_3_reservation_lock:
                if execution_id in cls._t30_3_reserved_execution_ids:
                    raise PermissionError("EABC_EXECUTION_ALREADY_RESERVED")
                cls._t30_3_reserved_execution_ids.add(execution_id)
